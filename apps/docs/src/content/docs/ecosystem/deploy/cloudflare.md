...
For a one-shot CLI run inside the same workerd environment:

```bash
npx flue run src/agents/translator.ts --message "Translate to French: Hello world"
```

Cloudflare's built-in Vite run environment is detected automatically and provides the generated Durable Object, Workers bindings, and persistent local state without requiring an authored HTTP mount. The CLI uses a random localhost-only route and removes it when the temporary server closes. Pass `--vite` to require the environment explicitly, or `--node` to bypass detection and force transport-free Node execution (where `cloudflare:*` imports fail).
