...
Like `app.ts`, the `db.ts` entry is discovered by convention — `vite dev`, `vite build`, and transport-free `flue run` all resolve it from the source root (`.flue/`, `src/`, or the project root) and connect it at startup. A Vite-hosted `flue run` uses its host's normal persistence instead (and Cloudflare rejects `db.ts` because Durable Object SQLite is built in). To place a Node database entry somewhere else, set the `db` path in your config file; see [Configuration](/docs/reference/configuration/#db).
...
| `flue run`   | Transport-free: `node_modules/.cache/flue/run.db`, never reset. Vite-hosted: the host's local persistence.               |
