...
This default is transport-free and Node-local, even when the project targets Cloudflare. When the agent needs its Vite host environment — for example Cloudflare Workers bindings or keyless Workers AI — add `--vite`. See [`flue run`](/docs/cli/run/) for the two execution modes.
