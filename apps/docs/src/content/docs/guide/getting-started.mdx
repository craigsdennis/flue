...
Flue automatically uses a Vite host environment when the project declares one; Cloudflare projects therefore run here through local workerd with Workers bindings and keyless Workers AI available. Projects without an automatic environment stay transport-free and Node-local. Use `--vite` to require a host or `--node` to bypass one; see [`flue run`](/docs/cli/run/) for details.
