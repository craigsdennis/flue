...
- **Transport-free [`flue run`](/docs/cli/run/) ignores the list.** Without `--vite`, it loads only the agent module and registers the full built-in set. A `--vite` run uses the generated host entry, so the configured list and host-specific providers apply normally.
