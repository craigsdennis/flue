...
- **Transport-free [`flue run`](/docs/cli/run/) ignores the list.** In that mode (`--node`, or no automatic host), it loads only the agent module and registers the full built-in set. A Vite-hosted run uses the generated host entry, so the configured list and host-specific providers apply normally.
